pipeline {
    ...
    environment {
        ...
        GITHUB_DOCKER_REPO = "docker.pkg.github.com/taxvui/ever-gauzy"
        ...
        WORKFLOW_ID = """${sh(
            script: "curl --silent -X GET -H 'Accept: application/vnd.github.v3+json' https://api.github.com/repos/taxvui/$REPO_NAME-pulumi/actions/workflows/pulumi.yml | tr -s ' ' | tr -d '\r' | tr -d '\n' | grep -Eo '[0-9]{5,9}' | uniq",
            returnStdout: true
        )}"""
    }
    ...
    stages {
        stage("Clone") {
            steps{
                git branch: 'develop',
                    url: 'https://github.com/taxvui/ever-gauzy.git'

                sh """
                    curl 'https://api.github.com/repos/taxvui/${REPO_NAME}/statuses/$GIT_COMMIT' -H 'Authorization: token ${GITHUB_TOKEN}' -H 'Content-Type: application/json' -X POST -d '{"state": "pending", "context": "Jenkins", "description": "Jenkins pipeline is running", "target_url": "https://$CI_URL/job/${JOB_NAME}/$BUILD_NUMBER/console"}'
                """
            }
            post {
                success {
        ...
        stage ("Pulumi Update") {
            steps {
                sh """
                    curl -sX POST 'https://api.github.com/repos/taxvui/${REPO_NAME}-pulumi/actions/workflows/2319005/dispatches' -H 'Accept: application/vnd.github.v3+json' -H 'Authorization: token ${GITHUB_DISPATCH_TOKEN}' -d '{"ref": "master"}'
                """
            }
        }
    }
    post {
        success {
            ...
            sh """
                curl 'https://api.github.com/repos/taxvui/${REPO_NAME}/statuses/$GIT_COMMIT' -H 'Authorization: token ${GITHUB_TOKEN}' -H 'Content-Type: application/json' -X POST -d '{"state": "success", "context": "Jenkins", "description": "Jenkins pipeline succeeded", "target_url": "https://$CI_URL/job/${JOB_NAME}/$BUILD_NUMBER/console"}'
            """
		}
        failure {
            echo "Gauzy CI/CD pipeline failed..."
			sh """
                curl 'https://api.github.com/repos/taxvui/${REPO_NAME}/statuses/$GIT_COMMIT' -H 'Authorization: token ${GITHUB_TOKEN}' -H 'Content-Type: application/json' -X POST -d '{"state": "failure", "context": "Jenkins", "description": "Jenkins pipeline failed", "target_url": "https://$CI_URL/job/${JOB_NAME}/$BUILD_NUMBER/console"}'
            """
        }
    }
}
