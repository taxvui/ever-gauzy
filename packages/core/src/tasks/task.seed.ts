...
export const createDefaultTask = async (
	...
	console.log(`${GITHUB_API_URL}/repos/taxvui/ever-gauzy/issues`);
	const issues$ = httpService
		.get(`${GITHUB_API_URL}/repos/taxvui/ever-gauzy/issues`)
		.pipe(map((response: AxiosResponse<any>) => response.data));
	const issues: any[] = await lastValueFrom(issues$);
	console.log(`Done ${GITHUB_API_URL}/repos/taxvui/ever-gauzy/issues`);
